...
def _init_worker(subset_df):
    global _SUBSET_DF
    _SUBSET_DF = subset_df

# Worker for multiprocessing
def _worker(task):
    (ii, drifter_lat, drifter_lon, freq_hz, ssp, drifter_depth,
   bot_ssp, bot_rho, bot_alpha) = task
    ...
    if ii % 500 == 0:
       print(f"[pid {os.getpid()}] ii={ii}", flush=True)
...
if __name__ == "__main__":
   ...
   print("start_method:", mp.get_start_method(), "cpu_count:", mp.cpu_count(), flush=True)
      ...
      for depth in propagationDepth:
             ...
             for dive_id in driftCTDsub['DiveID'].unique():
                  ...
                  if np.max(profile['depth'])>depth:
                      ...
                      tasks = [
                            (ii, drifter_lat, drifter_lon, freq_hz, ssp, drifter_depth, bot_ssp, bot_rho, bot_alpha)
                            for ii in range(len(subset_df))
                        ]
                      def _init_worker(subset_df_in):
                            global _SUBSET_DF
                            import os
                            print("PID:", os.getpid())
                            _SUBSET_DF = subset_df_in
                      ...
                      with ProcessPoolExecutor(
                          ...
                          for future in as_completed(futures):
                              ...
                              if ii % 50 == 0:
                                    print(f"Processed {ii}/{total_rows}", flush=True)
